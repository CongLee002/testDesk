...
if(isset($_POST['submit']) and $_POST['showinfo']!="" and $_POST['submit']=="提交"){
	...
	if($insert){
		echo "<script>alert ('添加成功！');window.location.href='grouphome.php'</script>";
		}else{
		echo "<script>alert ('添加失败！');window.location.href='grouphome.php'</script>";	
			}
		}else{
			echo "<script>alert ('信息不能为空！');window.location.href='grouphome.php'</script>";}
?>